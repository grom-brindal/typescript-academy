...
`console.log("hello world")` prints hello world. You can console log all sorts of words, play around!

<Editor
  id="2"
  modelsInfo={[{"filename":"allSorts.ts","value":"console.log(\"This 👌 tutorial 🔈🔈 is 💦 so 💯 fire 🔥🔥\")","language":"typescript"}]}
/>
...
Variable declarations in a module are isolated to that module. See that the file below is not a module. Duplicate declarations will cause errors,
`error` is a duplicate declaration because it was defined in `file2.ts` which is also not a module. There is no error for `noError` because it was 
previously defined in a module and it's declaration in that module is isolated.
...
## How does TypeScript work on the web?
The TypeScript compiler turns TypeScript into the equivalent JavaScript.
Web browsers run JavaScript, they do NOT run TypeScript, so this is needed to run the TypeScript in the browser. 
The formal term for this conversion process is "transpilation". Weird that a "compiler" "transpiles".
...
In our previous examples we put stuff in between quotation marks `"like this"` and either print it or assign it to a variable. 
Things in quotation marks are of type `string`, numbers are of type `number`. The technical term for a "thing" is a value.

<Editor
  id="8"
  modelsInfo={[{"filename":"logTypes.ts","value":"console.log(typeof \"I'm a string\")\r\nconsole.log(typeof 2)","language":"typescript"}]}
/>
