[Is there a way that when I am at Open-EdTech/typescript-academy that I can more easily navigate to typescript-academy/pages/book/ ? Being rather petty but a link at the top level would be nice as "SITE CONTENT" or something obvious.]

[Wait, this isn't located as a link at typescript.academy]

[Ah, found it at https://www.typescript.academy/book/Strings-and-Numbers]
...
There are different *types* of values, words like `"word"` or numbers like `2`. Technically the type of `"word"` is *string*, not word. 
We can ask for the type of a value. 
...
You might not want to write `console.log(typeof variable)` to find out the type of your variable. Fortunately, you can hover over a variable 
with your cursor to find the type of the variable.

<Editor
  id="15"
  modelsInfo={[{"filename":"hover.js","value":"let fromLet = \"from let\"\r\nconst fromConst = \"from const\"\r\nfromLet = \"reassign let\"\r\nconsole.log(fromLet)\r\nfromConst = \"this will cause an error\"","language":"javascript"}]}
/>

You should see this when you hover over `hoverString`:

<img
style={{margin: "0 auto"}}
  className="self-center flex-1 mt-0 mb-0"
  src="/JSTSbook/Variables/hoverInfo.png"
  alt="ts error"
/>
